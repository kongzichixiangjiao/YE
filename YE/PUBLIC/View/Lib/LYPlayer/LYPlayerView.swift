...
public class LYPlayerView: UIView {
    ...
    public override func layoutSubviews() {
        ...
        LYPlayer.videoProgress { (currentTime, cacheTime, state) in
            ...
            // 判断播放完成
            if (self.currentTimeLabel.text == self.totalTimeLabel.text) {
                if self.playAndPauseBtn.playStatus == .play {
                    self.isFinished()
                }
            }
    ...
    lazy var topShadeView: UIImageView = {
        ...
        let image = UIImage(named: "LYPlayer_top_shade")
    ...
    lazy var bottomShadeView: UIImageView = {
        ...
        let image = UIImage(named: "LYPlayer_bottom_shade")
    ...
    lazy var fullScreenBtn: UIButton = {
        ...
        fullScreenBtn.imageEdgeInsets = UIEdgeInsets(top: 10, left: 10, bottom: 10, right: 10)
        fullScreenBtn.setImage(UIImage(named: "LYPlayer_fullscreen"), for: .normal)
        fullScreenBtn.setImage(UIImage(named: "LYPlayer_shrinkscreen"), for: .selected)
    ...
    lazy var backBtn: UIButton = {
        ...
        backBtn.setImage(UIImage(named: "LYPlayer_back_full"), for: .normal)
        backBtn.imageEdgeInsets = UIEdgeInsets(top: 10, left: 15, bottom: 10, right: 15)
    ...
    lazy var lockScreenBtn: UIButton = {
        ...
        lockScreenBtn.setImage(UIImage(named: "LYPlayer_lock_nor"), for: .selected)
        lockScreenBtn.setImage(UIImage(named: "LYPlayer_unlock_nor"), for: .normal)
    ...
    //MARK: 播放和暂停按钮点击事件
    @objc func playAndPauseBtnAction(button: LYPlayButton) {
        if button.playStatus == .pause {
            if (self.totalTimeLabel.text == self.currentTimeLabel.text) {
                // 播放过再点播放
                self.progressSlider.value = 0.0
                let seconds = Double(0) / 1.0 * player.playerItem.duration.seconds
    ...
    //MARK: 全屏按钮点击事件
    @objc func fullScreenBtnAction(button: UIButton) {
        // 旋转控制器
        // 判断是否允许屏幕旋转
        if isAllowRotateScreen == false {
            print("当前全屏按钮处于锁定状态")
    ...
    //MARK: 返回按钮点击事件
    @objc func backBtnAction(button: UIButton) {
        releasePlayer()
    }
    ...
    //MARK: 锁屏按钮点击事件
    @objc func lockScreenBtnAction(button: UIButton) {
        isAllowRotateScreen = !isAllowRotateScreen
    }
    
    //MARK: 进度条被按下时的事件
    @objc func progressSliderTouchDownAction(slider: UISlider) {
        // 设置滑条正在被拖拽
        isSliderDragging = true
    }
    
    //MARK: 进度条手指抬起时的事件
    @objc func progressSliderTouchUpInsideAction(slider: UISlider) {
        // 设置滑条没有被拖拽
